...
- The NR runtime consumes **`DLSSNR.MVec`** in the DLSS convention: current → previous, in input pixels, scale **+1** (the same sign DLSS SR takes our field with). Measured against exact vectors on a synthetic pan, the estimated field keeps the output 2.5× more stable along the motion than zero vectors and preserves more detail; on handheld footage a RAFT sidecar at a 1024 px inference size beats zero vectors, the block matcher sits in between, and the old −1 default landed *below* zero vectors because it warped the history the wrong way. *Estimated* is therefore the default motion mode. `DLSSNR.Depth` is currently ignored by the runtime but stays wired for newer builds.
- **`DLSSNR.ControlMask` is an RGB weight map** (measured 2026-09-04 with graded bands on a still): R = master weight, G = tone weight, B = structure weight, A ignored; an all-ones mask is byte-identical to "no mask, automask off", so a bound mask *replaces* the runtime's automask, and a single-channel mask (what earlier builds bound) leaves G and B at zero and switches the whole effect off. The runtime exposes exactly one `ControlMask` and one set of `Intensity` / `LocalStructureStrength` / `LocalToneStrength` / `SkinStructureStrength` scalars per evaluate. NVIDIA's per-object "developer masking" (named groups, each with its own structure and tone) is therefore one composited mask, which is what the player does: GenMask's per-phrase layers (`<name>_mask_<phrase>.mp4`, up to four) get their own structure/tone sliders in the **Masks** panel group, the background its own pair, a **Feather** slider softens every layer's edge on the GPU by up to 32 output pixels so the effect fades in rather than stopping on the outline, *Show masks* tints each layer in the preview, and the exporter takes the same thing as `--mask-layer file:structure:tone`, `--mask-bg` and `--mask-feather`.
